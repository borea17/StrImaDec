...
from strimadec.models.modules import AIR_BaseClass


class testAIR_BaseClass(TestCase):

    BATCH_SIZE = 10

    def test_z_where_to_transformation_matrix_works_as_expected(self):
        rnd_z_where = 10 * torch.randn(testAIR_BaseClass.BATCH_SIZE, 3)
        trans_matrix_exp = torch.zeros(testAIR_BaseClass.BATCH_SIZE, 2, 3)
        for i_batch in range(testAIR_BaseClass.BATCH_SIZE):
        ...
        trans_matrix = AIR_BaseClass.z_where_to_transformation_matrix(rnd_z_where)
    ...
    def test_get_attention_rectangle_works_as_expected(self):
        ...
        source_rectangle = AIR_BaseClass.get_attention_rectangle(z_where_off_set_x_y, fake_img_size)
        self.assertTrue((source_rectangle_exp - source_rectangle).pow(2).sum() < 1e-12)
        return